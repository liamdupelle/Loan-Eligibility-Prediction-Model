...
# encode categorical features and prepare input features for prediction
if submitted and (rf_model is not None) and (sc is not None):
    ...
    # perform prediction
    @log_decorator
    def predict_eligibility(prediction_input):
    ...
    new_prediction = predict_eligibility(prediction_input_scaled)
    ...
    if new_prediction is not None:
        ...
        # notify the user that the prediction was not able to run successfully
        st.write("The Prediction was not able to run!")

st.write(
    """We used a machine learning (Random Forest Classifier) model to predict your eligibility, the features used in this prediction are ranked by relative
    importance below. The most important feature to the model is the credit history with an importance near 80 %."""
)
...
st.write(
    """The confusion matrix of the machine learning (Random Forest Classifier) model is shown below. The accuracy of the model is 85 %."""
)

st.write("**Confusion Matrix for Loan Eligibility Model (0 = Denial, 1 = Approval)**")
